...
def turbo(N=64):
    ...
    interp = interp1d(np.arange(256), np.array(turbo).T)
    return interp(np.arange(0, 256, int(256 / N))).T

    # x = max(0.0, min(1.0, N))
    # a = int(x * 255.0)
    # b = min(255, a + 1)
    # f = x * 255.0 - a
    # return [turbo[a][0] + (turbo[b][0] - turbo[a][0]) * f,
    #         turbo[a][1] + (turbo[b][1] - turbo[a][1]) * f,
    #         turbo[a][2] + (turbo[b][2] - turbo[a][2]) * f]
