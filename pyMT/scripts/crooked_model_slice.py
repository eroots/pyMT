...
for nudge_dist in [0]:
    for line in seisline:
        ...
        file_path = local_path + '/phd/NextCloud/Documents/ME_Transects/Swayze_paper/RoughFigures/seis2020/'
        file_name = 'Swayze_seis_linear_sincInterp'
        # if line in ['12', '14']:
        ...
        if overlay_seismic:
            ...
            ax.imshow(np.abs(seis_data), 
                      cmap='gray_r',
                      extent=[x_axis[0], x_axis[-1], min(dvec[-1], zlim[1]), dvec[0]],
                      alpha=1, interpolation='sinc')
        ax.set_aspect(aspect_ratio)
        if linear_xaxis:
            site_x = linear_site
