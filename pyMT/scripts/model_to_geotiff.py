...
model = DS.Model('E:/phd/NextCloud/data/Regions/MetalEarth/wst/mantle/finish/fromZ/wst-fromZ_lastIter.rho')
data = DS.RawData('E:/phd/NextCloud/data/Regions/MetalEarth/wst/j2/mantle/wst_mantle_try2.lst')
out_tag = 'E:/phd/NextCloud/data/ArcMap/WST/models/bg1000_finish/bg1000_finish'

models = ['mantle/bg300/finish/wst_bg300_finish_lastIter.rho',
		  'mantle/wNorth/wst-wNorth_lastIter.rho',
		  'mantle/wNorth/bg300/wstBG300_lastIter.rho',
		  'mantle/wNorth/embed_shield/hs300-1000/wst_hs300-1000_lastIter.rho',
		  'mantle/wNorth/embed_shield/hs300-1000/10km/wst_10km_lastIter.rho',
		  'mantle/wNorth/embed_shield/hs300-1000/10km/wStatic/0p5/wstStatic-0p5_NLCG_090.rho',
		  'mantle2crust/mantle2crust_lastIter.rho']
lists = ['mantle/wst_mantle_try2.lst',
		 'mantle/wst_mantle_wNorth.lst',
		 'mantle/wst_mantle_wNorth.lst',
		 'mantle/wst_mantle_wNorth.lst',
		 'mantle/wst_mantle_wNorth.lst',
		 'mantle/wst_mantle_wNorth.lst',
...
tags = ['bg300_finish',
	    ...
	    'wNorth_hs300',
	    'wNorth_hs300-1000',
	    'wNorth_hs300-1000_10km',
	    'wNorth_hs300-1000_10km_wStatic_0p5',
	    'mantle2crust-reg']
...
for ii in range(len(models)):
	...
	out_tag = 'E:/phd/NextCloud/data/ArcMap/WST/models/{}/{}'.format(tags[ii], tags[ii])
	model = DS.Model('E:/phd/NextCloud/data/Regions/MetalEarth/wst/' + models[ii])
	data = DS.RawData('E:/phd/NextCloud/data/Regions/MetalEarth/wst/j2/' + lists[ii])
	data.locations = data.get_locs(mode='lambert')
	# data.to_utm(17, 'N')

	# Check if the folder exists, if not, create it
	if not os.path.isdir('E:/phd/NextCloud/data/ArcMap/WST/models/' + tags[ii]):
		os.makedirs('E:/phd/NextCloud/data/ArcMap/WST/models/' + tags[ii])
	...
	for kind in ['X', 'Y', 'X-Y'][0:1]:
		...
		for iz in range(model.nz):
			# Only do it for depths > 10 km
			if model.dz[iz] > 10000:
				...
				outfile = out_tag + '_{:5.2f}km.tiff'.format(z[iz] / 1000)
